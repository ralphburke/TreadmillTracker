...
### Layout
The landing page consists of the application’s name "Treadmill tracker" as a heading. To centre this heading, I made the header element a flexbox with an h1 element centred inside. To keep the font large on mobile devices, I set the "white-space" property to "prewrap" to split the two words over two lines. I learned this method from https://developer.mozilla.org/en-US/docs/Web/CSS/white-space. The space character between the words was throwing off the centring so I put it in a span element and set its font size to 0 for mobile devices.
...
Because speed and incline values were not relevant to legs of 'rest' type, I created a function to disable them when 'rest' is selected. Originally, I used had event listener respond to every click with a check to see if 'rest' was selected. I didn’t want every click to run this code so I looked for a solution. From this page, https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/change_event,
I found the 'change' event, which worked well.
...
All button elements on my form were submitting the form when clicked.
https://stackoverflow.com/a/10836076
The above post clarified that all buttons are of submit type by default and that this could be overridden with type="button".
